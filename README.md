...
本项目是一个基于 Flask 的个人文献管理系统，面向课程期末验收场景，支持用户注册与审批、文献录入与检索、分类管理、BibTeX 导入导出、PDF 识别入库，以及字典清洗与 AI 日志辅助功能。本文档重点说明系统安装、使用、自动化测试与部署验证步骤，便于教师或评审人员快速完成环境搭建和功能验收。
...
`pytest` 已加入 [requirements.txt](/D:/GitHub项目/数据库项目/云端服务器版本/requirements.txt)，安装依赖后即可直接运行测试。
