...
def test_register_login_flow(client, app):
    resp = client.post(
        "/auth/register",
        data={"username": "alice", "password": "Password1!", "password2": "Password1!"},
...
def test_edit_document_without_new_attachment_does_not_create_upload_dir(
    client,
    app,
    approved_user_factory,
    login_as,
):
    user_id = approved_user_factory("editor-no-upload")
    login_as("editor-no-upload")

    with app.app_context():
        db.session.add(Document(user_id=user_id, title="Existing Paper"))
        db.session.commit()
    ...
    assert resp.status_code == 200
    assert b"Existing Paper Updated" in resp.data
    assert not os.path.exists(upload_dir)
...
def test_bulk_category_updates_selected_documents(client, app, approved_user_factory, login_as):
    ...
    resp = client.post(
        ...
        data={"doc_ids": ["1", "3"], "category_id": str(category_id), "return_page": "1"},
        follow_redirects=True,
    )
...
def test_bulk_category_can_clear_category(client, app, approved_user_factory, login_as):
    ...
    resp = client.post(
        "/documents/bulk-category",
        data={"doc_ids": ["1"], "category_id": "__none__", "return_page": "1"},
        follow_redirects=True,
    )
