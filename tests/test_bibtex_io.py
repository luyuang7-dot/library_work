...
from app.services.bibtex_io import export_bibtex, import_bibtex, import_single_entry, parse_entries
...
def _seed_user(app, username="u_imp"):
    with app.app_context():
        u = User(username=username, email=f"{username}@x.com", is_approved=True)
...
def test_import_single_entry_creates_document(app):
    ...
    with app.app_context():
        entry = parse_entries(
            "@article{k, title={Paper One}, year={2021}, author={Alice and Bob}, journal={Nature}, doi={10.1/abc}}"
        )[0]
...
def test_import_single_entry_skips_duplicate_by_doi(app):
    ...
    with app.app_context():
        ...
        db.session.commit()
        entry = parse_entries("@article{k, title={Different Title}, year={2021}, doi={10.1/dup}}")[0]
        result = import_single_entry(entry, uid)
        assert result["created"] is None
        assert result["skipped_reason"] is not None
...
def test_import_single_entry_with_category(app):
    ...
    with app.app_context():
        ...
        result = import_single_entry(entry, uid, category_id=cat.id)
        db.session.commit()
        assert result["created"].category_id == cat.id
...
def test_export_bibtex_contains_expected_fields(app):
    uid = _seed_user(app, "u_export")
    with app.app_context():
        created, skipped = import_bibtex(
            "@article{a, title={Export Me}, author={Alice and Bob}, journal={Nature}, year={2024}, doi={10.2/export}, keywords={数据库, 检索}}",
            uid,
        )
        assert created == 1
        assert skipped == 0
        docs = Document.query.filter_by(user_id=uid).all()
        exported = export_bibtex(docs)
        assert "Export Me" in exported
        assert "10.2/export" in exported
        assert "Alice and Bob" in exported
